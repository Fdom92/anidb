...
export class AnimeTrailer {
  ...
  componentWillLoad() {
    if ((document as any).documentMode || /Edge/.test(navigator.userAgent)) {
      (this.el.querySelector("#youtube-video") as HTMLElement).style.display =
        "none";
      (this.el.querySelector("#launch-video") as HTMLElement).style.display =
        "none";
      (this.el.querySelector("#background") as HTMLElement).style.display =
        "none";
      (this.el.querySelector("#mobile-video") as HTMLElement).style.display =
        "flex";
    }
  }
    ...
    return (
      this.trailer !== null && (
        <div class="anime-trailer">
          ...
          >
            <lazy-iframe
              src={"https://www.youtube.com/embed/" + this.trailer.id}
              text="Anime Trailer"
            />
          </div>
          <div
          ...
          <a
            href={"https://youtu.be/" + this.trailer.id}
            rel="noopener"
            id="mobile-video"
          >
            <span>See the trailer</span>
